...
//		File file =new File(classLoader.getResource(path).getFile());
		...
		try {
			...
			while ((rawStatus = reader.readLine()) != null) {
				// create tweet and send it to the queue
				Status status = DataObjectFactory.createStatus(rawStatus);
				m_tweetsQueue.offer(status); // add tweet into queue
			}
		} catch (FileNotFoundException ex) {
			LOG.error("Tried to read a file that does not exist", ex);
			LOG.trace("", ex);
		} catch (IOException ex) {
			LOG.error("IO Error while reading file", ex);
			LOG.trace("", ex);
		} finally {
			try {
				if (reader != null)
				...
				LOG.error("IO Error while cleaning up", ex);
				LOG.trace(null, ex);
			}
		}
	} catch (Exception ex) {
